...
class Encoder(nn.Module):
    ...
    def __init__(self, latent_dim, num_param, window_size, sample_layer=GaussianSample,
        ...
        self.fc_1 = nn.Linear(1, window_size)
...
class Decoder(nn.Module):
    """
        CNN 기반 디코더 모델
    """
    def __init__(self, latent_dim, num_param, out_window_size, scale_flag,
                 filter_1=8, filter_2=16, filter_3=32, filter_4=64):
        ...
        self.fc_1 = nn.Linear(out_window_size, 1)
        self.fc = nn.Linear(latent_dim, filter_4)
        self.deconv1_1 = nn.ConvTranspose2d(filter_4, filter_3, (num_param, int(np.floor(out_window_size/16))))
        self.bn1 = nn.BatchNorm2d(filter_3, track_running_stats=False)
        self.up1 = nn.Upsample(size=(num_param, int(np.floor(out_window_size/8))),
                               mode='bilinear')
        ...
        self.up2 = nn.Upsample(size=(num_param, int(np.floor(out_window_size/4))),
                               mode='bilinear')
        ...
        self.up3 = nn.Upsample(size=(num_param, int(np.floor(out_window_size/2))),
                               mode='bilinear')
        ...
        self.up4 = nn.Upsample(size=(num_param, out_window_size),
                               mode='bilinear')
        ...
        if scale_flag == 1:
            ...
            # self.output_activation = nn.PReLU(4096)
    ...
    def forward(self, x):
        ...
        x_out = torch.reshape(self.output_activation(self.convlast(x_out)), (-1, self.num_param, self.out_window_size))
        # x_out = torch.reshape(self.output_activation(self.convlast(x_out).transpose(1,3)).transpose(1,3), (-1, self.num_param, self.out_window_size))
        
        result = self.fc_1(x_out)
...
class CVAE(nn.Module):
    """
        CNN 기반 VAE 모델
    """
    def __init__(self, latent_dim, num_param, in_window_size, out_window_size, scale_flag):
        ...
        self.encoder = Encoder(self.latent_dim, self.num_param, self.in_window_size)
        self.decoder = Decoder(self.latent_dim, self.num_param, self.out_window_size, self.scale_flag)
        self.kl_div = 0
